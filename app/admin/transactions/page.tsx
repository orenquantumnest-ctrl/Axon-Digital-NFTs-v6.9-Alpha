...
// Pre-generate standard mock data as a robust offline/fail-safe fallback
function generateMockTransactions(): WalletTransactionRow[] {
  const types: Array<'DEPOSIT' | 'WITHDRAW' | 'REWARD' | 'TRANSFER' | 'FEE'> = ['DEPOSIT', 'WITHDRAW', 'REWARD', 'TRANSFER', 'FEE'];
  const currencies = ['BNB', 'USDT', 'AXON', 'USDC'];
  const statuses = ['COMPLETED', 'PENDING', 'FAILED', 'SUCCESS'];
  
  const mockRows: WalletTransactionRow[] = [];
  const baseTime = Date.now();
  for (let i = 0; i < 45; i++) {
    mockRows.push({
      id: `TX-${99100 + i}`,
      user_id: i % 2 === 0 && i > 0 ? "0x7bbc456c2c34972723cf03b9d44323d2539514de" : `user_0x${(123456 + i * 99).toString(16)}`,
      type: types[i % types.length],
      amount: Number(((15.5 + i * 12.3) % 450).toFixed(2)),
      currency: currencies[i % currencies.length],
      status: statuses[i % statuses.length],
      created_at: new Date(baseTime - i * 6 * 3600 * 1000).toISOString(),
      metadata: null
    });
  }
...
export default function AdminTransactionsPage() {
  ...
  // Core fetching effect with server-side range queries
  useEffect(() => {
    let active = true;
    async function fetchTransactions() {
      ...
      try {
        ...
        // Apply filters directly to Supabase
        if (statusFilter !== "ALL") {
        ...
        if (debouncedSearch) {
          const q = `%${debouncedSearch.toLowerCase().trim()}%`;
          query = query.or(`id.ilike.${q},user_id.ilike.${q},type.ilike.${q},currency.ilike.${q}`);
        }
        ...
        if (active) {
          if (data && !error) {
            ...
            // Fallback to offline mock filtering
            const mock = generateMockTransactions();
            const filtered = filterMockData(mock, debouncedSearch, statusFilter, minAmount, maxAmount, startDate, endDate);
  ...
  }, [currentPage, itemsPerPage, debouncedSearch, statusFilter, minAmount, maxAmount, startDate, endDate]);
      ...
      try {
        ...
        if (profile && !pError) {
          ...
          setClickedUserProfile({
            id: currentId,
            username: `U_${currentId.slice(0, 8)}`,
        ...
        if (txs && !txError) {
          ...
          // Fallback to custom/mock filter
          const fullMock = generateMockTransactions();
          setUserTransactions(fullMock.filter(t => t.user_id === currentId));
        }
  ...
  // Query helper to retrieve full matching rows for data exports
  async function fetchAllFilteredData(): Promise<WalletTransactionRow[]> {
    try {
      let query = supabase
        .from("wallet_transactions")
        .select("*");
    ...
    return filterMockData(generateMockTransactions(), debouncedSearch, statusFilter, minAmount, maxAmount, startDate, endDate);
  }
  ...
  const paginatedTxs = transactions; // Already sliced and queried server-side!
  ...
  const exportCSV = async () => {
    const dataToExport = await fetchAllFilteredData();
    let headers = "TX ID,Type,User ID,Amount,Currency,Status,Created At\n";
    let rows = dataToExport.map(tx => 
  ...
  const exportPDF = async () => {
    ...
    content += `                             TOTAL RECORDS: ${dataToExport.length}\n`;
    content += `================================================================================\n\n`;

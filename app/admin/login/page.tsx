    ...
    try {
      ...
      try {
        const { data, error: dbError } = await supabase
          .from("admin_accounts")
          .select("*")
          .eq("wallet_address", superWallet.trim().toLowerCase())
          .maybeSingle();
      ...
      // 2. Fallback to direct instructions credentials if DB fetch fails or has no record
      if (!adminRecord) {
        if (superWallet.trim().toLowerCase() === "0x7bbc456c2c34972723cf03b9d44323d2539514de") {
          adminRecord = {
